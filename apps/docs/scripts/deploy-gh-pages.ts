...
const DIST_DOCS = resolve(WORKSPACE_ROOT, 'apps', 'docs', 'dist');
const WORKTREE = resolve(WORKSPACE_ROOT, '.worktree-gh-pages');
const DATE = new Date().toISOString().split('T')[0];
const BRANCH = 'gh-pages';

function run(cmd: string, opts: Record<string, unknown> = {}): void {
  console.log(`  \x1b[36m  $\x1b[0m ${cmd}`);
  execSync(cmd, { cwd: WORKSPACE_ROOT, stdio: 'inherit', ...opts });
}

function capture(cmd: string, opts: Record<string, unknown> = {}): string {
  return execSync(cmd, { cwd: WORKSPACE_ROOT, encoding: 'utf-8', stdio: 'pipe', ...opts })
    .toString()
    .trim();
}
