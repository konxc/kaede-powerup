...
<div class="not-prose p-4 rounded-xl bg-kaede-surface border border-kaede-border mb-6">

**💡 Otomatis via CLI (disarankan)**

Sudah punya **API Key**? Lewati Method 1–2 — jalankan saja:

```bash
kaede auth login          # OAuth browser (auto) — token ditangkap otomatis
kaede auth login --manual # alternatif: print URL → buka → paste token
```

Token divalidasi (`/1/tokens`) dan disimpan ke `~/.config/kaede/secrets.env`.
Cek identitas: `kaede auth status`. Hapus: `kaede auth logout`.
