...
var SERVER = { name: "KAEDE Trello MCP", version: "1.0.0" };
var __dirname2 = dirname(fileURLToPath(import.meta.url));
var ROOT = resolve(__dirname2, "..");
...
async function trello(path, opts = {}) {
  const auth = getAuth();
  if (!auth)
    throw new Error("TRELLO_API_KEY or TRELLO_TOKEN not configured");
...
async function handleToolsCall(name, args) {
  switch (name) {
    ...
    case "create_board": {
      ...
      if (args.desc)
        body.desc = args.desc;
      if (args.idOrganization)
        body.idOrganization = args.idOrganization;
      if (args.defaultLabels !== undefined)
        body.defaultLabels = args.defaultLabels;
      if (args.defaultLists !== undefined)
        body.defaultLists = args.defaultLists;
    ...
    case "add_card_to_list": {
      ...
      if (args.dueDate)
        body.due = args.dueDate;
      if (args.start)
        body.start = args.start;
      if (args.labels)
        body.idLabels = args.labels.join(",");
    ...
    case "update_card_details": {
      ...
      if (args.start)
        body.start = args.start;
      if (args.dueComplete !== undefined)
        body.dueComplete = args.dueComplete;
      if (args.labels)
        body.idLabels = args.labels.join(",");
...
`);
  ...
  for (const line of lines) {
    ...
    } else if (method === "notifications/initialized") {} else if (method === "tools/list") {
      result(id, { tools: TOOLS });
    } else if (method === "tools/call") {
      handleToolsCall(params.name, params.arguments || {}).then((res) => {
        const content = typeof res === "string" ? [{ type: "text", text: res }] : [{ type: "text", text: JSON.stringify(res, null, 2) }];
        result(id, { content });
      }).catch((err) => {
