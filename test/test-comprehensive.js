...
async function main() {
  ...
  if (lists?.lists?.length) {
    await t('get_cards_by_list_id', () => client.callTool('get_cards_by_list_id', { listId: lists.lists[0].id }));
  }
  ...
  await t('attach_file_to_card', () =>
    client.callTool('attach_file_to_card', {
      cardId: CARD_ID,
  ...
  if (lists?.lists?.length) {
    const copied = await t('copy_card', () =>
      client.callTool('copy_card', {
        sourceCardId: CARD_ID,
        listId: lists.lists[0].id,
        name: 'TEST - copied card - cleanup me',
        keepFromSource: 'all',
      }),
    );
    if (copied?.id) {
  ...
  console.log('\n  \u250C\u2500\u2500\u2500 Checklists \u2500\u2500\u2500\u2510');
  const cl = await t('create_checklist', () =>
    client.callTool('create_checklist', { cardId: CARD_ID, name: 'Test Checklist' }),
  );
  if (cl?.id) {
    const item = await t('add_checklist_item', () =>
      client.callTool('add_checklist_item', { checklistId: cl.id, name: 'Test item' }),
    );
    await t('add_checklist_item 2nd', () =>
      client.callTool('add_checklist_item', { checklistId: cl.id, name: 'Second item' }),
    );
    await t('get_card_checklists', () => client.callTool('get_card_checklists', { cardId: CARD_ID }));
    if (item?.id) {
      await t('update_checklist_item', () =>
        client.callTool('update_checklist_item', { checklistId: cl.id, checkItemId: item.id, checked: true }),
      );
    }
    // delete last item
    ...
    if (items.length > 1) {
      await t('delete_checklist_item', () =>
        client.callTool('delete_checklist_item', { checklistId: cl.id, checkItemId: items[items.length - 1].id }),
      );
    }
    // copy checklist
  ...
  if (members?.members?.length) {
    await t('assign_member_to_card', () =>
      client.callTool('assign_member_to_card', { cardId: CARD_ID, memberId: members.members[0].id }),
    );
    await t('remove_member_from_card', () =>
      client.callTool('remove_member_from_card', { cardId: CARD_ID, memberId: members.members[0].id }),
    );
  }

  console.log('\n  \u250C\u2500\u2500\u2500 Labels \u2500\u2500\u2500\u2510');
  await t('get_board_labels', () => client.callTool('get_board_labels', { boardId: BOARD_ID }));
  const label = await t('create_label', () =>
    client.callTool('create_label', { boardId: BOARD_ID, name: 'TEST - cleanup me', color: 'red' }),
  );
  if (label?.id) {
  ...
  await t('update_label', async () => {
    ...
    return client.callTool('update_label', {
      labelId: r.labels[0].id,
      name: r.labels[0].name,
      color: r.labels[0]?.color || 'blue',
    });
  });
  ...
  console.log('\n  \u250C\u2500\u2500\u2500 Lists \u2500\u2500\u2500\u2510');
  const newList = await t('add_list_to_board', () =>
    client.callTool('add_list_to_board', { boardId: BOARD_ID, name: 'TEST - cleanup me' }),
  );
  if (newList?.id) {
  ...
  if (lists?.lists?.length) {
    await t('update_list', () =>
      client.callTool('update_list', { listId: lists.lists[0].id, name: lists.lists[0].name }),
    );
