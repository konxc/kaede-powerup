...
import type { PlaybookResult, PlanStep, BoardSnapshot, EnforceResult } from './types';

const PUBLIC_DIR = resolve(import.meta.dir, '..', 'apps', 'web', 'dist');
...
export async function startApiServer(port: number = 3456): Promise<http.Server> {
  ...
  return new Promise((resolvePromise) => {
    server.listen(port, '0.0.0.0', () => {
      ...
      console.log(`  \x1b[90m  GET  /              — Web UI (apps/static)\x1b[0m`);
...
async function handleTool(
  ...
  enforcePlaybook: (plan: PlanStep[], playbook: PlaybookResult, boards?: BoardSnapshot[]) => EnforceResult,
  parsePlaybook: (content: string) => PlaybookResult,
): Promise<void> {
  ...
  switch (name) {
    case 'enforce_playbook': {
      ...
      const result = enforcePlaybook(plan || [], parsePlaybook(pb), boards || []);
      res.writeHead(200, { 'Content-Type': 'application/json' });
